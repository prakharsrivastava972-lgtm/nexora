    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-4 items-center">
          <button
            onClick={toggleTheme}
            className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white px-3 py-1.5 rounded text-sm border border-slate-300 dark:border-slate-700"
          >
      ...
      {loading && <p className="text-slate-500 dark:text-slate-400">Loading recommendations...</p>}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {continueLearning.map((item) => (
              <div
                ...
                className="bg-white dark:bg-slate-800 rounded-xl p-6 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-750 transition-colors border-l-4 border-emerald-500 shadow-sm dark:shadow-none"
              >
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {recommendations.map((item) => (
          <div
            key={item.item_id}
            onClick={() => navigate(`/items/${item.item_id}`)}
            className="bg-white dark:bg-slate-800 rounded-xl p-6 flex flex-col cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-750 transition-colors shadow-sm dark:shadow-none"
          >
            ...
            <div className="flex gap-2 mt-auto">
              ...
              <button
                ...
                className="flex-1 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-900 dark:text-white py-2 rounded text-sm font-medium"
              >
                Save
              </button>
              <button
