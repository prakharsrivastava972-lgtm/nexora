...
function NavSidebar({ open, onClose }) {
  ...
  useEffect(function () {
    if (open) {
      ...
      const timer = setTimeout(function () { setAnimateIn(true); }, 20);
      return function () { clearTimeout(timer); };
    } else {
      setAnimateIn(false);
      const timer = setTimeout(function () { setShouldRender(false); }, 300);
      return function () { clearTimeout(timer); };
    }
  }, [open]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      <div
        className={
          "w-72 bg-white dark:bg-slate-800 h-full shadow-xl flex flex-col transition-transform duration-300 ease-out " +
          (animateIn ? "translate-x-0" : "-translate-x-full")
        }
      >
        <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-700">
          <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">NEXORA</span>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-xl leading-none transition-transform hover:scale-110">×</button>
        </div>
                ...
                {continueLearning.slice(0, 4).map(function (item) {
                  return (
                    <button
                      ...
                      className="w-full text-left text-sm text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 py-1.5 truncate block transition-colors"
                    >
                      {item.title}
                    </button>
                  );
      ...
      <div
        className={
          "flex-1 bg-black/30 transition-opacity duration-300 ease-out " +
